...
fn test_parser_parsepath() {
  ...
  match templ {
    ...
    Some(template) => {
      ...
      assert!(bs[7]  == ~Block{class: Text       , content: ~"<p class=\"foobar\">\n  "    , pos: Pos{line_no: 6}});
      assert!(bs[8]  == ~Block{class: Print      , content: ~" n + 1 "                     , pos: Pos{line_no: 6}});
      assert!(bs[9]  == ~Block{class: Text       , content: ~"\n"                          , pos: Pos{line_no: 7}});
      assert!(bs[10] == ~Block{class: Text       , content: ~"</p>\n"                      , pos: Pos{line_no: 8}});
      assert!(bs[11] == ~Block{class: Code       , content: ~" } "                         , pos: Pos{line_no: 8}});
      assert!(bs[12] == ~Block{class: Text       , content: ~"\n"                          , pos: Pos{line_no: 9}});
      assert!(bs[13] == ~Block{class: Text       , content: ~"</html>\n"                   , pos: Pos{line_no: 10}});
    }
  }
}
